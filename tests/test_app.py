...
# Local
from app import (
    _default_classical_input_frame,
    classify_posting,
...
def _build_classical_pipeline():
    ...
    metadata) on synthetic rows, mirroring test_explain.py."""
    train_df = pd.DataFrame(
        {
            ...
            "required_experience": (
                ["Mid-Senior level"] * len(TRAIN_TEXTS)
            ),
            "required_education": (
                ["Bachelor's Degree"] * len(TRAIN_TEXTS)
            ),
...
def _build_lstm_pipeline():
    """Build a tiny LstmPipeline, mirroring test_explain.py's LSTM
    fixture."""
...
def test_default_classical_input_frame_has_expected_columns_and_defaults():
    """The classical fallback frame has all ten expected columns,
    with department/industry/function fixed to missing."""
...
def test_predict_fraud_probability_uses_text_only_for_lstm_pipeline():
    """An LstmPipeline is called with the cleaned text alone, not a
    metadata frame."""
    lstm_pipeline = _build_lstm_pipeline()
    probability = predict_fraud_probability(
        lstm_pipeline, "urgent wire transfer scam"
    )
...
def test_predict_fraud_probability_uses_default_frame_for_classical_pipeline():
    """A classical pipeline is called with the fixed-default
    metadata frame and returns a valid probability."""
    classical_pipeline = _build_classical_pipeline()
    probability = predict_fraud_probability(
        classical_pipeline, "urgent wire transfer scam"
    )
...
def test_classify_posting_returns_none_for_empty_input():
    """Empty or punctuation-only input is rejected before the model
    is ever touched."""
